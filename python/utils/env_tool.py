...
def decode_session_jwt(session_token: Optional[str]) -> Optional[Dict[str, Any]]:
    """
    Decode the payload of a LEETCODE_SESSION JWT.

    Parameters:
        session_token (str): Raw LEETCODE_SESSION value (header.payload.signature).

    Returns:
        dict: Decoded payload, or None when the token is not a decodable JWT.
    """
    ...
    try:
        ...
        return None
    # A JWT payload is a JSON object; anything else is not a usable session.
    return payload_data if isinstance(payload_data, dict) else None


def get_session_identity(cookie: Optional[str]) -> Optional[Dict[str, Any]]:
    """
    Extract the logged-in LeetCode account from the LEETCODE_SESSION JWT.

    The JWT payload carries the account identity, so no network request is
    needed to tell which account a cookie belongs to.

    Parameters:
        cookie (str): The cookie string to inspect. Should contain LEETCODE_SESSION.

    Returns:
        dict: {'user_slug', 'username', 'user_id'} with None for absent fields,
              or None when the cookie carries no decodable LEETCODE_SESSION.
    """
    if not cookie:
        return None
    ...
    payload_data = decode_session_jwt(session_match.group(1))
    if not payload_data:
        return None

    return {
        'user_slug': payload_data.get('user_slug'),
