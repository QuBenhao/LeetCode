    ...
    def write_solution(self, code_default: str, code: str = None, problem_id: str = "",
        ...
        for i, line in enumerate(code_default.split("\n")):
            ...
            if line.startswith("func "):
                ...
                its.append(GolangWriter.__process_inputs(code_default, line.split("(")[1].split(")")[0], structs_map))
                ...
                for tmp in code_default.split("\n"):
                    ...
                    if tmp.startswith("func ") and (
                        ...
                        tp0, tp1, tp2, tp3 = GolangWriter.__process_inputs(code_default,
                                                                           tmp.split("(")[1].split(")")[0],
                                                                           structs_map, True)
                        rt = tmp.split("{")[0].split(")")[-1].strip()
                        structs_map[struct_name]["construct"] = (tmp.split("(")[0].split("func ")[-1].strip(),
                                                                 (tp0, tp1, tp2,
                                                                  tp3.replace("inputValues", "opValues[0]")),
                                                                 rt)
        ...
        with open(file_path, 'r', encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                if import_part:
                    ...
                    import_part = True
                    continue
                if "func Solve(input string) interface{} {" in line or \
                        "func Solve(inputJsonValues string) interface{} {" in line:
                    break
                    ...
                    case "*Node":
                        if ("Left *Node" in code_default
                            ...
                            for var in vrs:
                                ...
                                json_parse.append(
                                    f"\tif err := json.Unmarshal([]byte(inputValues[{i}]), &" + f"arr{i}" +
                                    "); err != nil {\n\t\tlog.Fatal(err)\n\t}\n")
