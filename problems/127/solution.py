...
class Solution(solution.Solution):
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        if endWord not in wordList:
            return 0
        front, back = ProirtyQueue(), ProirtyQueue()
        front.__add__((1, 0, beginWord))
        back.__add__((1, 0, endWord))
        while front and back:
            if len(front) > len(back):
                ts, ot, target = back, front, beginWord
            else:
                ts, ot, target = front, back, endWord
            l, _, s = ts.pop()
            for i in range(len(s)):
                for c in string.ascii_lowercase:
                    t = s[:i] + c + s[i + 1:]
                    if t in wordList and t not in ts:
                        if t in ot:
                            return l + ot.explored[t]
                        ts.__add__((l + 1, heuristic(t, target), t))
        return 0
