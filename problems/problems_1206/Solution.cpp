...
class Skiplist {
	...
	SkipNode* searchWithUpdate(int target, vector<SkipNode*>& update) {
		...
		for (int i = level; i >= 0; --i) {
			...
			update[i] = current; // each update[i] will point to the last node before the target
		}
		return current->forward[0];
	}
public:
    explicit Skiplist(): level(0) {
    ...
    void add(int num) {
		...
		for (int i = 0; i <= newLevel; ++i) {
			newNode->forward[i] = update[i]->forward[i]; // link new node's forward pointers with current next nodes
			update[i]->forward[i] = newNode; // link update nodes' forward pointers to new node
		}
    }
    ...
    bool erase(int num) {
		...
		if (current != nullptr && current->val == num) {
			for (int i = 0; i <= level; ++i) {
				if (update[i]->forward[i] != current) { // no need to continue as we have already erased the node
					break;
				}
				update[i]->forward[i] = current->forward[i]; // unlink the node from the skiplist
			}
			delete current;

			while (level > 0 && head->forward[level] == nullptr) { // If the highest level is empty, reduce the level
				level--;
			}
			return true;
		}
